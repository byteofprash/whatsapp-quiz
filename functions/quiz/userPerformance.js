...
exports.handler = function (context, event, callback) {
  ...
  base(context.EVENTS_TABLE_NAME)
    .select({
    ...
    .firstPage((err, events) => {
      ...
      if (events.length < 1) {
        ...
        events.forEach((event) => {
          userChoices.push({
            questionNum: event.fields.question_num,
            quizNum: event.fields.quiz_num,
            userChoice: event.fields.user_choice,
          });
        ...
        Promise.allSettled(promises)
          .then((questions) => {
            questions.forEach((question) => {
              if (question.value) {
                let yourAnswer = userChoices.filter(
                  (choice) =>
                    choice.questionNum == question.value.question_num &&
                    choice.quizNum == question.value.quiz_num
                )[0].userChoice;
