...
// Sorting function
const simpleComparator = (a, b, ascending, columnId) => {
...
export const SortableColumnsDataGrid = ({ handleSelectedUser }) => {
  ...
  let getData = async () => {
    ...
    let scoreData = data
      .map((result) => [result.phone, result.name, result.total_points])
      .sort((a, b) => simpleComparator(a, b, false, 2));
    setSortedData(scoreData);
    let scores = [...new Set(data.map((result) => result.total_points))].sort(
      (a, b) => {
        return b - a;
      }
    );
  ...
  const handleSortingColumn = (columnId) => {
    ...
    const newSortedColumns = sortedColumns.map(() => "none");
